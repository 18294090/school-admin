...
for  dirpath, dirnames, filenames in os.walk(os.path.join(os.getcwd(),"app","static",'abnormal_paper','17')): #遍历答题卷文件夹阅卷
    for filepath in filenames:
        print(filepath)
        n+=1        
        ep=judge.open2(os.path.join(dirpath, filepath))
        ep=judge.paper_ajust(img,ep)
        split=judge.paper_split(ep,12,[70,80])
        s=judge.check_select(split[1],12)
